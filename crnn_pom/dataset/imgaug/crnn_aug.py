...
class CRNNTransform(object):
    ...
    def __init__(self, size: Union[int, Tuple[int, int]], keep_ratio_with_pad: bool):  # (H, W)
    ...
    def __call__(self, image):
        if self._keep_ratio_with_pad:
            ...
            transform = Compose(
                transform=[Resize((self._imgH, resized_w)), Normalize(), PAD(input_channel, self.imgH, self.imgW)]
            )
        else:
            transform = Compose(transforms=[Resize((self._imgH, self._imgW)), Normalize()])
